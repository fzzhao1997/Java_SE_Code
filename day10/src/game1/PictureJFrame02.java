...
public class PictureJFrame02 extends JFrame {
    public  PictureJFrame02() {//Java程序编译器是根据代码结构来进行编译处理的，执行的时候也是根据代码结构来处理的。
    ...
    public int[][]  paintView() {
        ...
        int [][] array = arr(4,4);//同一类中非静态直接调用静态或非静态方法都行
    ...
    //定义二维数组,用来储存图片的编号
    public  int[][] arr(int x, int y) {//循环需在方法里
